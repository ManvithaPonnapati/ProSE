...
class Sequence:
    ...
    async def get_msa(self):
        af2_prediction_json = await post_sample_request(
            f"{ALPHAFOLD_URL}/alphafold/msa",
            {
                "sequence": self.sequence
            },
            )
        return af2_prediction_json
    ...
    async def fold_sequence(self):
        ...
        return af2_prediction_json['results']['predicted_output'], af2_prediction_json['results']['plddt']
    ...
    async def get_msa(self):
        ...
        return esm_prediction_json

    async def get_mcmcmc_samples(self,constraints:list[int]=[],num_samples:int=10,temperature:float=0.1):
        """
    ...
    async def get_mcmc_samples(self,constraints:list[int]=[],num_samples:int=1,temperature:float=0.1):
        """
        Get MCMC samples from the sequence by masking the sequence at all locations but the constrained residues
